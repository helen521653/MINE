...
class MutualInformationEstimator(pl.LightningModule):
    def __init__(self, x_dim, z_dim, loss='mine', **kwargs):
        super().__init__()
        self.x_dim = x_dim
        self.T = CustomSequential(ConcatLayer(), nn.Linear(x_dim + z_dim, 128), nn.ReLU(),
        ...
        self.avg_test = 0.0
        self.test_outputs = []
        #self.save_hyperparameters() # for TensorBoard
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_mi', mi, prog_bar=True)
        #writer.add_scalar("Loss/train", loss)
        #writer.add_scalar("MI/train", mi)


        self.log('rho', self.kwargs.get('current_rho', 0.0))
        self.log('true_mi', self.kwargs.get('true_mi', 0.0))
        return loss
    ...
    def on_test_epoch_end(self):
        ...
        self.avg_test = torch.stack([x['test_mi'] for x in self.test_outputs]).mean().item()
        self.log("avg_test", self.avg_test)
        self.log("true_mi", self.kwargs.get('true_mi', 0.0))
